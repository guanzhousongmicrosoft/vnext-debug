...
LOG_FILE="$LOG_DIR/cosmos-test-macos-$TIMESTAMP.log"
...
check_docker_macos() {
    log "Checking Docker Desktop status on macOS..." "INFO"
    
    # Check if Docker Desktop app is running
    if ! pgrep -f "Docker Desktop" > /dev/null; then
        log "Docker Desktop is not running. Attempting to start..." "WARN"
        open -a Docker
        ...
        while ! docker info > /dev/null 2>&1 && [[ $count -lt $max_wait ]]; do
            sleep 5
            ((count += 5))
            log "Waiting for Docker Desktop to start... ($count/$max_wait seconds)" "INFO"
        done
        
        if ! docker info > /dev/null 2>&1; then
            log "Failed to start Docker Desktop within $max_wait seconds" "ERROR"
            return 1
        fi
    fi
    
    log "Docker Desktop is running" "INFO"
...
test_cosmos_emulator() {
    ...
    # Add Cosmos DB emulator configuration optimized for macOS
...
try
{
    // Configure Cosmos DB emulator with macOS-specific settings
    var cosmos = builder
        ...
        .RunAsPreviewEmulator(options =>
        {
            ...
            options.WithEnvironment("AZURE_COSMOS_EMULATOR_IP_ADDRESS_OVERRIDE", "127.0.0.1");
            options.WithEnvironment("DOCKER_DEFAULT_PLATFORM", "linux/amd64");
            // Disable additional services that might cause issues on macOS
    ...
    app.Services.GetRequiredService<ILogger<Program>>()
        .LogInformation("Starting Aspire application with Cosmos DB emulator on macOS...");
    
    app.Run();
}
catch (Exception ex)
{
    Console.WriteLine($"Failed to start application on macOS: {ex}");
    throw;
}
EOF

    # Create test API project (same as Ubuntu but with macOS-specific logging)
    ...
    # Create controller with macOS-specific diagnostics
    mkdir -p Controllers
...
public class CosmosTestController : ControllerBase
{
    ...
    public async Task<IActionResult> TestConnection()
    {
        ...
        try
        {
            _logger.LogInformation("=== Starting Cosmos DB connection test on macOS ===");
            
            // Add macOS-specific environment info
            testResults["platform"] = "macOS";
            testResults["architecture"] = RuntimeInformation.ProcessArchitecture.ToString();
            testResults["os_description"] = RuntimeInformation.OSDescription;
            ...
            {
                _logger.LogError(ex, "Failed to list databases on macOS");
            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        _logger.LogInformation("Query successful on macOS. Items: {Count}, RU: {RequestCharge}", 
                            results.Count, results.RequestCharge);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex, "Query test failed on macOS");
                        testResults["query_test"] = new { 
                            Status = "Failed", 
                            Error = ex.Message,
                            Type = ex.GetType().Name,
                ...
                {
                    _logger.LogError(ex, "Failed to access Users container on macOS");
                    testResults["container_access"] = new { 
                        Status = "Failed", 
                        Error = ex.Message,
                        Type = ex.GetType().Name,
            ...
            {
                _logger.LogError(ex, "Failed to access MyDb database on macOS");
                testResults["database_access"] = new { 
                    Status = "Failed", 
                    Error = ex.Message,
                    Type = ex.GetType().Name,
                ...
                // Check for the specific schema issue
                if (ex.Message.Contains("cosmos_api") || ex.Message.Contains("schema"))
                {
                    _logger.LogCritical("FOUND THE ISSUE on macOS: Schema-related error detected!");
                    testResults["schema_issue_detected"] = true;
                    testResults["platform_specific"] = "Issue occurs on macOS as well";
                }
            }
            ...
            _logger.LogInformation("=== Cosmos DB connection test completed on macOS ===");
            
            return Ok(testResults);
        }
        catch (Exception ex)
        {
            _logger.LogCritical(ex, "Critical error during Cosmos DB test on macOS");
...
EOF
    ...
    # Configure services (same as Ubuntu)
    cat > Program.cs << 'EOF'
...
app.Logger.LogInformation("TestApi starting up on macOS...");
...
EOF
    ...
    export DOCKER_DEFAULT_PLATFORM="linux/amd64"  # For M1/M2 Macs
    ...
    for i in {1..15}; do  # Longer wait for macOS
        sleep 5
        log "Startup progress: ${i}/15 ($(($i * 5)) seconds)" "INFO"
    ...
    local max_retries=25  # More retries for macOS
    local retry_count=0
    ...
    while [[ $retry_count -lt $max_retries && "$test_success" != "true" ]]; do
        ...
        if [[ -n "$api_url" ]]; then
            ...
            if curl -f -s -m 30 "$api_url" -o "$response_file" 2>/dev/null; then
                ...
                if command -v jq &> /dev/null; then
                    cat "$response_file" | jq '.' | tee -a "$LOG_FILE"
                else
                    cat "$response_file" | tee -a "$LOG_FILE"
                fi
                
                # Check if the response indicates success
                if command -v jq &> /dev/null && jq -e '.overall_status == "Success"' "$response_file" > /dev/null 2>&1; then
                    test_success=true
                    log "Cosmos DB test completed successfully!" "INFO"
                elif command -v jq &> /dev/null && jq -e '.schema_issue_detected == true' "$response_file" > /dev/null 2>&1; then
                    log "ISSUE REPRODUCED on macOS: Schema 'cosmos_api' does not exist error detected!" "ERROR"
        ...
        if [[ $retry_count -lt $max_retries ]]; then
            sleep 12  # Longer sleep for macOS
        fi
    done
    ...
    if [[ "$test_success" == "true" ]]; then
        ...
        netstat -an | grep LISTEN | tee -a "$LOG_FILE"
...
# Main execution
log "Starting Cosmos DB Emulator tests on macOS" "INFO"
log "Debug Level: $DEBUG_LEVEL, Test Scenario: $TEST_SCENARIO" "INFO"
log "Environment: $(uname -a)" "INFO"
log "Architecture: $(uname -m)" "INFO"

# Check if running on Apple Silicon
if [[ "$(uname -m)" == "arm64" ]]; then
    log "Running on Apple Silicon (M1/M2/M3)" "INFO"
...
if $issue_reproduced; then
    log "Issue successfully reproduced on macOS!" "INFO"
    exit 2
elif $overall_success; then
    log "All tests PASSED on macOS (Issue NOT reproduced)" "INFO"
    exit 0
else
    log "Tests FAILED on macOS" "ERROR"
    exit 1
fi
